...
install.packages(c("ggplot2","dplyr","stringr","scales","xlsx","data.table","tidyr","ggpubr","purrr", "gridExtra","broom","lazyeval"))
r1<-c("ggplot2","dplyr","stringr","scales","xlsx","data.table","tidyr","ggpubr","purrr", "gridExtra","broom","lazyeval")
...
#create bar chart for a categorical variable grouped by another categorical variable, y axis will show the percentage of count of subgroup
#Sample code: https://stackoverflow.com/questions/24776200/ggplot-replace-count-with-percentage-in-geom-bar

##############test plot###############################################
#create a dataframe contain bivariate test results for two categorical variables

#create a dataframe contain the results of chisquare test
#reference for referencing column in defined function: https://stackoverflow.com/questions/7310186/function-in-r-passing-a-dataframe-and-a-column-name
plot_chisq <- function (df, x, z){
  
              fit=chisq.test(df[, x], df[, z])  #what does fit do?
          ...
          df_sum <- df %>%
              ...
              return(df_sum) %>%
              ggplot(aes_string(x, y = Perc, fill = z))+
              geom_bar(stat = "identity", position = "dodge")+
              theme(panel.grid = element_blank(),
              legend.position = c(0.9,0.9),
              legend.background = element_blank()) +
              annotate("text", x=5, y=0.28, label = paste("χ2 = ", round(fit["Statistic"],2), "; p = ", round(fit["p.value"],2)))  

}

plot_chisq(merged_working, x="Sex", z="DC.mRS")


#test for one categorical variable and a continuouse variable
#calculate group means
means<-aggregate(merged_working$age_at_visit ~ merged_working$DC.mRS, mean)

merged_working %>%
  filter(!is.na(age_at_visit) & !is.na(DC.mRS)) %>%
  ggplot(aes(x=DC.mRS, y=age_at_visit))+
  geom_boxplot()+
...
test2<-
  merged_working %>%
  do(fit = kruskal.test(.$DC.mRS~.$age_at_visit)) %>%
  tidy(fit)

merged_working %>%
  filter(!is.na(age_at_visit) & !is.na(DC.mRS)) %>%
  group_by(age_at_visit, DC.mRS) %>%
  summarise(count=n()) %>%
  mutate(Perc=count/sum(count)) %>%
  ggplot(aes(x=DC.mRS, y=age_at_visit))+
  geom_boxplot()+
  theme(panel.grid = element_blank(),
        legend.position = c(0.9,0.9),
        legend.background = element_blank()) +
  annotate("text", x=5, y=0.28, label = paste("Krskal-Wallis = ", round(test2$statistic,2), "; p = ", round(test2$p.value,3)))
